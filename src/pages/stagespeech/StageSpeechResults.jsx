...
export default function StageSpeechResults() {
  ...
  const { samples = [], transcript = "", topic } = state;
  ...
  useEffect(() => {
    ...
    fetchSessionHistory(user.userId).then((history) => {
      if (cancelled) return;
      const previous = (history || []).find((s) => s.mode === "stagespeech");
      ...
      try {
        ...
        const res = await fetch(`${API_BASE}/api/analyze-session`, {
          ...
          body: JSON.stringify({
            ...
            behavioralSummary,
            mode: "stage speech",
            context: `Delivering a stage speech on the topic: ${topic || "N/A"}`,
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center">
        <div className="text-center">
          <p className="text-lg mb-2">Analyzing your speech...</p>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <main className="max-w-3xl mx-auto px-6 py-10">
        <h1 className="text-2xl font-semibold mb-1">Stage speech results</h1>
