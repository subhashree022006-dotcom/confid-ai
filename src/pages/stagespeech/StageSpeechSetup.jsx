...
export default function StageSpeechSetup() {
  ...
  function start() {
    ...
    navigate("/stagespeech/session", { state: { topic, fileName: file?.name || null, mode } });
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <main className="max-w-xl mx-auto px-6 py-10">
        <h1 className="text-2xl font-semibold mb-1">Set up your stage speech</h1>
        <p className="text-gray-400 mb-6">Upload notes or slides if you have them, then deliver your speech to the camera.</p>
        <div className="space-y-4 rounded-2xl border border-white/10 bg-white/[0.02] p-6">
          <div>
            ...
            <input className="mt-1 w-full rounded-lg bg-white/5 border border-white/10 px-3 py-2" value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="e.g. Why sustainability matters" />
          </div>
          <div>
            <label className="text-sm text-gray-300">Upload notes / image (optional)</label>
          ...
          <div>
            <label className="text-sm text-gray-300 block mb-2">How will you deliver it?</label>
          ...
          <button onClick={start} className="w-full py-2.5 rounded-lg bg-gradient-to-r from-blue-500 to-cyan-400 text-slate-950 font-semibold hover:opacity-90">Start speech</button>
        </div>
      </main>
    </div>
  );
