...
export default function PresentationResults() {
  ...
  useEffect(() => {
    ...
    fetchSessionHistory(user.userId).then((history) => {
      if (cancelled) return;
      const previous = (history || []).find((s) => s.mode === "presentation");
  ...
  const overall = confidence !== null
    ? computeOverallScore({ confidence, eyeContact, gesture, communication: explanation })
    : computeOverallScore({ eyeContact, gesture, communication: explanation });
